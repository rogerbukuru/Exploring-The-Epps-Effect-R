...
# This script walks one through creating a 480 volume bucket Derman Framwork aggregated data from TAQ data 
...
correlation_estimators = get_estimated_correlations(derman_480_volume_bucket)
...
ggplot(data = melted_corr, aes(Var1,Var2 , fill = value))+
  ...
  theme_bw()+ 
  labs(title = "(a) MM 480 Volume Bucket")+
...
ggplot(data = melted_corr, aes(Var1,Var2 , fill = value))+
  ...
  scale_fill_gradientn(colours = color,limit=c(-1,max(stock_correlations)),name=TeX("$\\rho$")) +
  theme_bw()+ 
  labs(title = "(a) HY 480 Volume Bucket")+
  annotate("text", x = 7, y = 3, label = TeX("$\\bar{| \\rho_{ij} | } = 0.1411"), size = 6) + 
  theme(legend.key.size = unit(1.7, "cm"),
        legend.key.width = unit(0.5,"cm"),
        legend.title = element_text(size = 13, face = "bold"),
